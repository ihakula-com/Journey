...
- (BOOL)doCallHttpService:(NSString *)serviceName withParameters:(NSDictionary *)paraDic andServiceUrl:(NSString *)serviceUrl forDelegate:(id)theDelegate
{
    ...
    NSAssert(theDelegate != nil, @"doCallService:delegate is empty");
...
- (BOOL)checkNetwork {
    if (!self.sysNetworkMonitor.isReachable) {
        UIAlertView *aalert = [[UIAlertView alloc] initWithTitle:@"Meddelande" message:self.sysNetworkMonitor.networkTrafficInfo delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [aalert show];
        [aalert release];
        return NO;
    }
